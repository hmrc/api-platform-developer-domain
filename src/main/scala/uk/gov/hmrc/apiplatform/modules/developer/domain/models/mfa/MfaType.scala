...
object MfaType {
  ...
  def unsafeApply(text: String): MfaType = {
    apply(text).getOrElse(throw new RuntimeException(s"$text is not a valid MFA Type"))
  }

  import play.api.libs.json.Format
  import uk.gov.hmrc.apiplatform.modules.common.domain.services.SealedTraitJsonFormatting

  implicit val format: Format[MfaType] = SealedTraitJsonFormatting.createFormatFor[MfaType]("MFA Type", apply(_))
}
